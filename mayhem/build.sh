...
# ── LeakSanitizer off, at BUILD time (fleet policy: preventive, every ASan target) ──
# `-Zsanitizer=address` always bundles LSan in; there is no flag keeping ASan's
# memory-corruption checks while dropping just leak detection. The only sanctioned
# off-switch is a strong __lsan_is_turned_off() linked into the binary
# (mayhem/lsan_off.c) — never a runtime disable/enable wrap, never a compiled-in
# default-options override. ASan and UBSan stay on and halting.
#
# rustc takes a single -Clinker, so we point it at a wrapper that prepends the object
# to every fuzz link. The clean KAT probe below builds with RUSTFLAGS unset, so it
# never sees this.
if [ -n "$RUST_SAN" ]; then
  LSAN_OBJ=/tmp/mayhem-lsan_off.o
  # -gdwarf-3 LAST: $SANITIZER_FLAGS ends with a plain -g and clang-19 reads that as
  # DWARF-5. This object is prepended to the link, so its CU lands FIRST in
  # .debug_info — a DWARF-5 one here fails the §6.2 item 10 gate for the whole binary.
  ...
  export RUSTFLAGS="$RUSTFLAGS -Clinker=$LINK_WRAPPER"
  echo "LSan disabled at build time via $LSAN_OBJ (__lsan_is_turned_off)"
fi
